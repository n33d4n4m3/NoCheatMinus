...
# Verbose
execute as @a[limit=1,sort=random] if score @s ncmVerbose matches 1 run tellraw @a[scores={ncmOperator=2}] [{"text":""},{"text":"NCM","color":"red"},{"text":": Verbose logging is now enabled for: ","color":"gray"},{"selector":"@a[scores={ncmVerbose=1}]","color":"yellow"},{"text":".","color":"gray"}]
execute as @a[limit=1,sort=random] if score @s ncmVerbose matches 1 run tellraw @a[scores={ncmInputR=0}] [{"text":""},{"text":"NCM","color":"red"},{"text":": Currently, you don't see any check-fail notifications. If you want to see the check-fail notifications of players with enabled verbose-mode only, you have to change your input mode to 1 with ","color":"gray"},{"text":"/trigger ncmInput set 1","color":"yellow","click_event":{"action":"run_command","command":"/trigger ncmInput set 1"},"hover_event":{"action":"show_text","value":[{"text":"/trigger ncmInput set 1"}]}}]
execute as @a[limit=1,sort=random] if score @s ncmVerbose matches 1 run tellraw @a[scores={ncmInputR=2}] [{"text":""},{"text":"NCM","color":"red"},{"text":": Currently, you see check-fail notifications of all players. If you want to see the check-fail notifications of players with enabled verbose-mode only, you have to change your input mode to 1 with ","color":"gray"},{"text":"/trigger ncmInput set 1","color":"yellow","click_event":{"action":"run_command","command":"/trigger ncmInput set 1"},"hover_event":{"action":"show_text","value":[{"text":"/trigger ncmInput set 1"}]}}]

execute as @a if score @s ncmVerbose matches 1 run scoreboard players set @s ncmVerbose 2
execute as @a[limit=1,sort=random] if score @s ncmVerbose matches 0 run tellraw @a[scores={ncmOperator=2}] [{"text":""},{"text":"NCM","color":"red"},{"text":": Verbose logging is now disabled for: ","color":"gray"},{"selector":"@a[scores={ncmVerbose=0}]","color":"yellow"},{"text":".","color":"gray"}]
